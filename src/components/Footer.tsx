  ...
  return (
    <footer className="mt-24 text-ink-200" style={{ background: 'linear-gradient(180deg, #0C4A6E 0%, #082F49 100%)' }}>
      <div className="container-x py-14">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {PRODUCTS.map((p) => (
                <li key={p.slug}><Link href={`/products/${p.slug}`} className="text-brand-100/80 hover:text-white">{p.shortTitle}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {topApps.map((a) => (
                <li key={a.slug}><Link href={`/applications/${a.slug}`} className="text-brand-100/80 hover:text-white">{a.shortTitle}</Link></li>
              ))}
              <li><Link href="/applications" className="text-brand-200 hover:text-white font-medium">View all →</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              <li><Link href="/about" className="text-brand-100/80 hover:text-white">About Us</Link></li>
              <li><Link href="/quality" className="text-brand-100/80 hover:text-white">Quality & QC</Link></li>
              <li><Link href="/custom-printed-pouches" className="text-brand-100/80 hover:text-white">Custom Printed</Link></li>
              <li><Link href="/blog" className="text-brand-100/80 hover:text-white">Blog</Link></li>
              <li><Link href="/faq" className="text-brand-100/80 hover:text-white">FAQ</Link></li>
              <li><Link href="/contact" className="text-brand-100/80 hover:text-white">Contact</Link></li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-10 pt-8 border-t border-brand-800/60">
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            {[
              ...
              <a key={label} href={href} target="_blank" rel="noopener" className="px-3 py-1.5 rounded-full bg-white/5 hover:bg-brand-500/30 border border-white/10 hover:border-brand-400/40 transition text-brand-100">
                {label}
              </a>
            ))}
          </div>
        ...
        <div className="mt-10 pt-6 border-t border-brand-800/60 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-brand-200/70">
